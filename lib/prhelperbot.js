	...
	_createClass(PRHelperBot, [{
		...
		value: function sendMessage(_ref) {
			...
			if (action === 'opened' || action === 'reopened') {
				this.prBot.postMessageToGroup((0, _helpers.getChannelName)(repo), '', openedAttachment(prData));
			} else if (action === 'closed' && prData.merged) {
				this.prBot.postMessageToGroup((0, _helpers.getChannelName)(repo), '', closedAttachment(prData));
			}
		}
	}]);
