...
var closedAttachment = function closedAttachment(prData) {
  return {
    ...
    as_user: "true",
    attachments: [{
      fallback: (0, _helpers.getSlackName)(prData.merged_by.login) + " merged " + (0, _helpers.getSlackName)(prData.user.login) + "'s pull request",
      color: "#6F42C1",
      text: (0, _helpers.getSlackName)(prData.merged_by.login) + " merged " + (0, _helpers.getSlackName)(prData.user.login) + "'s pull request. You rock!"
    }]
  };
};
